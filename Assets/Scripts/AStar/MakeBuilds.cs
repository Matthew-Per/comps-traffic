    ...
    IEnumerator Assembly()
    {
        ...
        while (inProgress)
        {
            ...
            if (cState == 1)
            {
                ...
                foreach (Direction d in Direction.Directions)
                {
                    ...
                    if (cellLead.hasCell(translatedCell))
                    {
                        ...
                        possibles.Add(new PossibilitiesContainer(translatedCell, d.Opposing(), choiceScript));
                    }
                }
            }
            while (cState == 1)
            ...
            while (cState == 2)
            {
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    if (cellLead.hasCell(selectorPos))
                    {
                        ...
                        {
                            var found = possibles[i];
                            homeExitDirection = found.directionTowards.Opposing();//entrance is inwards to home exit is outwards to street
            ...
            if (cState == 4)
            {
                ...
                foreach (Direction d in Direction.Directions)
                {
                    ...
                    if (cellLead.hasCell(translatedCell))
                    {
                        ...
                        var choice = Instantiate(PossibleChoiceFab, vec3, Quaternion.identity);
                        var choiceScript = choice.GetComponent<PossibleCell>();
                        destpossibles.Add(new PossibilitiesContainer(translatedCell, d.Opposing(), choiceScript));
                    }
                }
            }
            while (cState == 4)
            ...
            while (cState == 5)
            {
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    if (cellLead.hasCell(selectorPos))
                    {
                        ...
                        {
                            var found = destpossibles[i];
                            destExitDirection = found.directionTowards.Opposing();//entrance is inwards to home exit is outwards to street
            ...
            if (cState != 6)
            {
                ...
                Cell[] newCells = cellLead.CreateBuiding(homeVec,DestVec,homeExitDirection,homeEntrance,destEntrance,destExitDirection);
                //build.Setup(aStar,)
                cState = 0;
                inProgress = false;
                selector.Follow();
